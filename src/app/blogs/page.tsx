...
function BlogSection() {
  ...
  const { items, loading, error } = useFetch("http://localhost:1337/api/blogs");
    ...
    <div className={styles.Blog_Outer}>
      <div className={styles.Blog_left}>
        ...
        <p className={styles.intro}>
          ...
          Company Website Blog. Join us in a journey through in-depth articles,
          practical tutorials, and thought-provoking discussions as we delve
          into the latest advancements, coding intricacies, and software
          development strategies. Our blog is your gateway to staying informed
          and inspired in the dynamic realm of technology and innovation
        </p>
        <button className={styles.Blog_button}>Read more blogs{brokenArrow}</button>
      </div>
      <div className={styles.Blog_right}>
          <BlogCard />
      </div>
      <button className={styles.Blog_button_2}>Read more blogs{brokenArrow}</button>
    </div>
    </div>
  );
}
